	...
	public void run() { //All game logic, variable initializations, and window control
        while (true) {
            try {
                ...
                foxyInitialize.Foxy();
                hallAAnimation(Office.monitorUp, Window.cameraLocation);
            } catch(Exception e){
            }
        }
    }
	public void hallAAnimation(boolean x, String y) {
        if (x && y.equals("West Hall A")) { //Handle the animation of foxy running down the hall
            if (FoxyTimer < 4) {
                FoxyTimer++;
            }
            if (FoxyTimer == 4) {
